...
> [!NOTE] We disable Ollama in the cluster by default. Most developers connect to their own LLM provider (see Step 2).
...
modelType: ollama
modelId: qwen3.5:4b
# No secret needed — but set OLLAMA_BASE_URL in the agent config
```

> [!NOTE] Ollama runs on your machine, not in the cluster. The agent connects via `host.docker.internal:11434` (Colima/Docker Desktop) or your machine's IP.
