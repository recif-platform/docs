...
## Two Ways to Use Récif

This is the key insight: **Corail agents are autonomous**. They don't need Récif to run. This enables two distinct deployment models that serve different needs:

![Two Access Modes](/docs/diagrams/two-modes.svg)

### Mode 1: Direct Access — Developer / Integration
...
### Mode 2: Platform Access — Enterprise / Mass Adoption

Users interact through the Récif dashboard or API. The platform proxies requests to agents and adds governance, monitoring, and evaluation. This is ideal for:
...
The magic is that **both modes coexist**. The same agent can be accessed directly by a CI/CD pipeline AND through the platform dashboard by employees. Récif doesn't lock you in — it layers governance on top of autonomous agents.
